...
const DATA_FILE_PATH = `${__dirname}/../dataDepot/data.json`;
...
const loadSavedTODOList = () => {
  ...
  return TodoList.load(JSON.parse(fs.readFileSync(DATA_FILE_PATH, 'utf8') || '[]'));
};
...
const deleteItem = (req, res) => {
  ...
  fs.writeFile(DATA_FILE_PATH, JSON.stringify(todoList.getAllTodo()), () => {});
...
const serveAllTodo = (req, res) => {
  const extension = DATA_FILE_PATH.split('.').pop();
